...
import src.artists as artists_db
import src.releases as releases_db
from src.artists import Artist
...
def remove(query: str = typer.Argument(help="Artist name or browse ID")):
    ...
    if typer.confirm(f"Remove {selected.name}?"):
        artists_db.remove(selected.browse_id)
...
def list_artists():
    ...
    console.print(table)
    console.print(f"\n[dim]{len(all_artists)} artist(s) followed.[/dim]")


@app.command()
def sync():
    """Check for new releases and manually select them."""
    all_artists = artists_db.load()
    ...
    for artist in all_artists:
        ...
        table = Table("", "Title", "URL", show_header=True, header_style="bold")
    ...
    console.print()
    if untrack_urls:
        releases_db.archive_releases(untrack_urls)
    ...
    if download_urls:
        ...
        for url in download_urls:
            console.print(url)
        confirmation = typer.confirm("Have you downloaded all release(s)?")
        if confirmation:
            releases_db.archive_releases(download_urls)
